...
public class StmtPoint {
				...
				for (Unit unit : us) {
					if (unit instanceof Stmt) {
						if (((Stmt) unit).containsInvokeExpr()) {
							if (((Stmt) unit).getInvokeExpr().getMethod() == node.getSmthd()) {			//Error
	...
	public static List<StmtPoint> findSetter(SootField sootField) {	//为sootField找到改变过它的值的语句
		List<StmtPoint> sps = new ArrayList<StmtPoint>();
		...
		if (mthdes != null) {
			for (SootMethod mthd : mthdes) {
				PatchingChain<Unit> us = mthd.retrieveActiveBody().getUnits();//找到给sootField赋值的语句
				for (Unit unit : us) {
					if (unit instanceof Stmt) {
						for (ValueBox vbox : ((Stmt) unit).getDefBoxes()) {
							if (vbox.getValue() instanceof FieldRef && ((FieldRef) vbox.getValue()).getField() == sootField) {
								cbg = BlockGenerator.getInstance().generate(mthd.retrieveActiveBody());//如果是系统类，找到该block
								block = BlockUtility.findLocatedBlock(cbg, unit);
